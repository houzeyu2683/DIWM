...
# y, e = m(x)
# z = [e.unsqueeze(0) for i in range(6)]
# torch.cat(z, 0).shape
# layer['backbone'](x).shape
'''
...
class image(nn.Module):
    ...
    def forward(self, x='(2,3,224,224)'):
        ...
        v['embedding'] = self.layer['embedding'](v['backbone'].mean(0))
        pass
        
        y = v['backbone'], v['embedding']
        return(y)
...
class text(nn.Module):
    ...
    def __init__(self, vocabulary=None):
        ...
        layer = dict()
        layer['vector'] = nn.Embedding(self.vocabulary.size, 512)
        layer['position'] = nn.Embedding(self.vocabulary.size, 512)
        self.layer = nn.ModuleDict(layer)
        return
...
class attention(nn.Module):
    ...
    def __init__(self, vocabulary=None):
        ...
        layer = dict()
        layer['decoder'] = nn.TransformerDecoder(
            decoder_layer=nn.TransformerDecoderLayer(1024, 8),
            num_layers=4,
            norm=None
        )
        layer['output'] = nn.Linear(1024, self.vocabulary.size)
        self.layer = nn.ModuleDict(layer)
        return
    ...
    def forward(self, x=('image', 'text')):
        ...
        image, text = x
        v = dict()
        v['memory'], v['embedding'] = self.image(image)
        v['memory'] = v['memory'].permute(2,1,0)
        v['text'] = self.text(text)
        v['embedding'] = torch.cat([v['embedding'].unsqueeze(0) for _ in range(len(v['text']))], 0)
        v['embedding'] = torch.cat([v['text'], v['embedding']], 2)

        v['decoder'] = self.layer['decoder'](
            tgt = v['embedding'],
            memory = v['memory'],
...
class model(nn.Module):
    ...
    def predict(self, x='image', device='cpu', limit=30):
