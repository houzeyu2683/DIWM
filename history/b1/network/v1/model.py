...
class image(nn.Module):
    ...
    def __init__(self):
        ...
        backbone = nn.Sequential(*[i for i in backbone.children()][:-4], nn.Flatten(2))
        pass
        
        layer = dict()
        layer['1'] = backbone
        layer['2'] = nn.Sequential(nn.Linear(784, 128), nn.Tanh())
        layer['3'] = nn.TransformerEncoder(
            encoder_layer=nn.TransformerEncoderLayer(d_model=128, nhead=8),
            num_layers=1,
            norm=None
        )
        layer['4'] = nn.Linear(128, 300)
...
class text(nn.Module):
    ...
    def forward(self, x=None):
        ...
        v['1'] = self.layer['1'](
            ...
            src_key_padding_mask = mask.padding(x=x, value=self.vocabulary.index['<padding>'] if(self.vocabulary) else 1)
        )
        pass
...
class attention(nn.Module):
    ...
    def forward(self, x=('image', 'text')):
        ...
        v['1'] = self.layer['1'](
            ...
            memory_mask = None, 
            tgt_key_padding_mask = mask.padding(x=text, value=self.vocabulary.index['<padding>'] if(self.vocabulary) else 1),
...
def optimizer(model=None):
    ...
    if(model):
        ...
        function = torch.optim.Adam(model.parameters(), lr=1e-3, betas=(0.9, 0.98), eps=1e-9)
        return(function)
