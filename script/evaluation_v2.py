...
machine = network.v3.machine(model=model, optimizer=optimizer, cost=cost, device='cuda', folder='log(v1)/', checkpoint=0)

machine.load(path='log(v2)/model-12.checkpoint', what='model')
evaluation = dict()
